...
<c:url var="addUrl" value="/ctl/help" />

<c:url var="addSearch" value="/ctl/help/search" />

<c:url var="editUrl" value="/ctl/help?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Help Report</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/help/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header bgcolor"
				style="color: white;">Help List</h5>
			<div class="card-body">
				<div class="row g-3">
				
				<s:bind path="requestId">
						<div class="col">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="rm" varStatus="help">
							<tr>
							
								<td scope="row">${help.index+1}</td>
								<td scope="row">${rm.requestId}</td>
								<td scope="row">${rm.issue}</td>
								<td scope="row">${rm.date}</td>
								<td scope="row">${rm.description}</td>
								<td scope="row">${rm.comment}</td>
								<c:if test="${sessionScope.user.roleId == 1}">

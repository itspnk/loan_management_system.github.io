...
<c:url var="addUrl" value="/ctl/updateLoanVerification" />

<c:url var="addSearch" value="/ctl/updateLoanVerification/search" />

<c:url var="editUrl" value="/ctl/updateLoanVerification?id=" />

<c:url var="statusUrl" value="/ctl/updateLoanVerification/status?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i>Update Loan Verification Report</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/updateLoanVerification/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header bgcolor"
				style="color: white;">Update Loan Verification Report</h5>
			<div class="card-body">
				<div class="row g-3">
		...
		<div class="table-responsive">
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="us" varStatus="updateLoanVerification">
							<tr>
							<c:if test="${sessionScope.user.roleId == 3}">
							<td ><input type="checkbox" class="case"
									name="ids" value="${us.id}"></td>
									</c:if>
								<td scope="row">${updateLoanVerification.index+1}</td>
								...
								<td scope="row"><a href="${pageContext.request.contextPath}/ctl/updateLoanVerification/getFile?id=${us.id}">Report</a></td>

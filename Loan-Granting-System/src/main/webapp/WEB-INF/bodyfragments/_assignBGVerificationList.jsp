...
<c:url var="addUrl" value="/ctl/assignBGVerification" />

<c:url var="addSearch" value="/ctl/assignBGVerification/search" />

<c:url var="editUrl" value="/ctl/assignBGVerification?id=" />

<c:url var="statusUrl" value="/ctl/assignBGVerification/status?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i>Assign For BG Verification Report</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/assignBGVerification/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header bgcolor"
				style="color: white;">Assign For BG Verification Report</h5>
			<div class="card-body">
				<div class="row g-3">
		...
		<div class="table-responsive">
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="us" varStatus="assignBGVerification">
							<tr>
							<c:if test="${sessionScope.user.roleId == 1}">
							<td ><input type="checkbox" class="case"
									name="ids" value="${us.id}"></td>
									</c:if>
								<td scope="row">${assignBGVerification.index+1}</td>

...
<c:url var="addUrl" value="/ctl/updateBGVerification" />

<c:url var="addSearch" value="/ctl/updateBGVerification/search" />

<c:url var="editUrl" value="/ctl/updateBGVerification?id=" />

<c:url var="statusUrl" value="/ctl/updateBGVerification/status?id=" />

<c:url var="loanSearchUrl" value="/ctl/applyLoan/search?lid=" />

<c:url var="updateUrl" value="/ctl/updateBGVerification?aId=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i>Update BG Verification Report</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/updateBGVerification/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header bgcolor"
				style="color: white;">Update BG Verification Report</h5>
			<div class="card-body">
				<div class="row g-3">
					...
					<s:bind path="fieldOfficerName">
						<div class="col">
							<sf:input path="${status.expression}" class="form-control form-control-sm"
								placeholder="Search By Field Officer Name" />
						</div>
					</s:bind>
					</c:if>
				...
				<table class="table table-bordered border-primary">
					<thead>
						...
						<th scope="col"><input type="checkbox" id="selectall">Select
							...
							<th scope="col">Verification Update</th>
							<th scope="col">Description</th>
							<th scope="col">Report</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${list}" var="us" varStatus="updateBGVerification">
							<tr>
							<c:if test="${sessionScope.user.roleId == 4}">
							<td ><input type="checkbox" class="case"
									name="ids" value="${us.id}"></td>
									</c:if>
								<td scope="row">${updateBGVerification.index+1}</td>
								<td scope="row"><a href="${loanSearchUrl}${us.loan.id}">${us.loanRequestId}</a></td>
								<td scope="row">${us.loan.name}</td>
								<c:if test="${sessionScope.user.roleId == 1}">
								<td scope="row">${us.fieldOfficerName}</td>
								
								</c:if>
								<td scope="row">${us.verificationUpdate}</td>
								<td scope="row">${us.description}</td>
								<td scope="row"><a href="${pageContext.request.contextPath}/ctl/updateBGVerification/getFile?id=${us.id}">Report</a></td>
